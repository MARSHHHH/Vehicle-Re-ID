...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir',default='.\VeRi\pytorch',type=str, help='training dir path')
    parser.add_argument('--train_all', action='store_true', help='use all training data' )
    parser.add_argument('--batchsize', default=64, type=int, help='batchsize')#todo 32
    ...
    parser.add_argument('--total_epoch', default=60, type=int, help='total training epoch')#todo 60
    ...
    name = opt.name
    # Check if CUDA (GPU support) is available and the user's specified GPU IDs
    ...
    for str_id in str_ids:
        ...
        if gid >=0:
            gpu_ids.append(gid)
    opt.gpu_ids = gpu_ids
    # If there are available GPUs, set the default tensor type and enable GPU acceleration
    if len(gpu_ids)>0:
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                                 ...
                                                 prefetch_factor=2, num_workers=2, persistent_workers=True)#todo num_workers=2, persistent_workers=True # 8 workers may work faster
                  for x in ['train', 'val']}
    ...
    if opt.DG:
        ...
        dataloaders['DG'] = torch.utils.data.DataLoader(image_datasets['DG'], batch_size = max(8, opt.batchsize//2),
                                                 shuffle=True, pin_memory=True, num_workers=2)#todo num_workers=2
    ...
    # Initialize dictionaries to track loss and error for training and validation
    y_loss = {} # loss history
    y_loss['train'] = []
    y_loss['val'] = []
    y_err = {}
    ...
    def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
        ...
        # best_acc = 0.0
        # Define warm-up parameters for learning rate
        warm_up = 0.1  # We start from the 0.1*lrRate
        warm_iteration = round(dataset_sizes['train'] / opt.batchsize) * opt.warm_epoch  # first 5 epoch
        # Initialize loss functions for various losses
        if opt.arcface:
